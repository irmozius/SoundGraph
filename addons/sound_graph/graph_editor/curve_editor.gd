...
func _ready() -> void:
	...
	if _curve == null:
		...
		_curve.domain_changed.connect(_on_curve_changed)
...
func set_curve(curve: Curve):
	_curve = curve
	#min_x = _curve.min_domain
	#max_x = _curve.max_domain
	queue_redraw()
	...
	if event is InputEventMouseButton and event.pressed:
		...
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			if selected_index != -1:
				...
				var new_pos: Vector2 = get_world_pos(mpos).clamp(Vector2(_curve.min_domain, _curve.min_value), Vector2(_curve.max_domain, _curve.max_value))
...
func update_view_transform() -> void:
	...
	var world_rect: Rect2 = Rect2(min_x, min_y, max_x - min_x, max_y - min_y)
...
func _draw():
	...
	var min_edge: Vector2 = get_world_pos(Vector2(_curve.min_domain, size.y))
	var max_edge: Vector2 = get_world_pos(Vector2(size.x, _curve.min_value))
	...
	var step_size: Vector2 = Vector2(abs(_curve.max_domain - _curve.min_domain) / 4.0, abs(_curve.max_value - _curve.min_value) / 2.0)
	...
	draw_line(Vector2(_curve.min_domain, min_edge.y), Vector2(_curve.min_domain, max_edge.y), grid_color_primary)
	draw_line(Vector2(_curve.max_domain, max_edge.y), Vector2(_curve.max_domain, min_edge.y), grid_color_primary)
	...
	for i in range(grid_steps.x):
		var x = _curve.min_domain + i * step_size.x
		draw_string(
			font, 
			get_view_pos(Vector2(x, _curve.min_value)) + Vector2(0, font_height - round(2 * _editor_scale)),
			String.num(x, 2), 
	...
	for i in range(grid_steps.y + 1):
		...
		draw_string(
			font,
			get_view_pos(Vector2(min_x, y)) + Vector2(2, -2),
			String.num(y, 2),
...
func plot_curve_accurate(step: float, scaling: Vector2):
	...
	# Edge Lines
	draw_line(Vector2(min_x, first_point.y) * scaling + Vector2(0.5, 0.0), first_point * scaling, line_color)
	draw_line(Vector2(min_x, last_point.y) * scaling - Vector2(-1.5, 0), last_point * scaling, line_color)
